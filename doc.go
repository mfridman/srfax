...
There are a few notable differences between the official docs and this client:

1.	As a result of variable types being returned in the Result field, see note about error handling: https://github.com/mfridman/srfax#srfax

2.	Response format supports JSON-only. Despite the API being able to return both XML
and JSON, it is a design decision (for now) to support JSON only. As a result,
